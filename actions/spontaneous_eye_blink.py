...
class SpontaneousEyeBlinkAction(BaseAction):
    ...
    def detect_peak_frame(self, landmarks_seq, w, h):
        ...
        for idx in valid_indices:
            ...
            eye_openings.append(avg_opening)

        # 找局部最小值
        min_idx = 0
        min_value = float('inf')

        for i in range(1, len(eye_openings) - 1):
            if (eye_openings[i] < eye_openings[i-1] and
                eye_openings[i] < eye_openings[i+1]):
                if eye_openings[i] < min_value:
                    min_value = eye_openings[i]
                    min_idx = i

        if min_idx == 0:
            min_idx = np.argmin(eye_openings)
